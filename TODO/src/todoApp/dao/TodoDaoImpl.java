...
// DB 연결하고 각 기능에 맞게 작업, 데이터베이스 todos테이블에 CRUD 작업
	@Override
	...
	public Todo selectTodo(long todoId) {
		...
		try {
			...
			if(rs.next()) { // 결과가 있을 경우에 값을 저장, 없는데 저장하면 에러 발생
	...
	public List<Todo> selectAllTodos() {
		...
		try {
			...
			// 결과가 여러 줄일 경우 while() 사용하여 처리, 1줄일때는 if()
	...
	public boolean deleteTodo(long todoId) {
		String DELETE_USER_SQL = "DELETE FROM todos WHERE id=?";
	...
	public boolean updateTodo(Todo todo) {
		...
		String UPDATE_USER_SQL = "UPDATE todos SET title = ?, username= ?, description =?, target_date =?, is_done = ? WHERE id = ?";
		...
		try {
			...
			rowUpdated = pstmt.executeUpdate() > 0; // 결과가 없는 업데이트, 삭제, 입력 등은 쿼리 업데이트 한 줄의 갯수가 리턴됨
